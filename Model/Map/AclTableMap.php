...
/**
 * This class defines the structure of the 'acl' table.
 *
 *
 *
...
class AclTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'CustomerGroupAcl.Model.Map.AclTableMap';
     ...
     */
    const OM_CLASS = '\\CustomerGroupAcl\\Model\\Acl';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'CustomerGroupAcl.Model.Acl';
     ...
     */
    const CREATED_AT = 'acl.CREATED_AT';

    /**
     * the column name for the UPDATED_AT field
     */
    const UPDATED_AT = 'acl.UPDATED_AT';
    ...
    public function initialize()
    {
        ...
        $this->setName('acl');
        $this->setPhpName('Acl');
        $this->setClassName('\\CustomerGroupAcl\\Model\\Acl');
    ...
    public function buildRelations()
    {
        ...
        $this->addRelation('CustomerGroupAcl', '\\CustomerGroupAcl\\Model\\CustomerGroupAcl', RelationMap::ONE_TO_MANY, array('id' => 'acl_id', ), 'CASCADE', null, 'CustomerGroupAcls');
        $this->addRelation('AclI18n', '\\CustomerGroupAcl\\Model\\AclI18n', RelationMap::ONE_TO_MANY, array('id' => 'id', ), 'CASCADE', null, 'AclI18ns');
    } // buildRelations()
    ...
    /**
     * Method to invalidate the instance pool of all tables related to acl     * by a foreign key with ON DELETE CASCADE
     */
    ...
    {
        return $withPrefix ? AclTableMap::CLASS_DEFAULT : AclTableMap::OM_CLASS;
    }
     ...
     * @return array (Acl object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = AclTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = AclTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + AclTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = AclTableMap::OM_CLASS;
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            AclTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = AclTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = AclTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(AclTableMap::DATABASE_NAME)->getTable(AclTableMap::TABLE_NAME);
    }
    ...
    {
      $dbMap = Propel::getServiceContainer()->getDatabaseMap(AclTableMap::DATABASE_NAME);
      if (!$dbMap->hasTable(AclTableMap::TABLE_NAME)) {
        $dbMap->addTableObject(new AclTableMap());
      }
    }

    /**
     * Performs a DELETE on the database, given a Acl or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or Acl object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \CustomerGroupAcl\Model\Acl) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(AclTableMap::DATABASE_NAME);
            $criteria->add(AclTableMap::ID, (array) $values, Criteria::IN);
        }

        $query = AclQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { AclTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { AclTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return AclQuery::create()->doDeleteAll($con);
    }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Acl object
        }
